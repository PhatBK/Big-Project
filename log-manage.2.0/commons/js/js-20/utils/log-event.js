...
const logging_host = {
    // 'host': 'http://192.168.1.18:9001/web/all/event/log-datas',
    ...
    'host': 'http://192.168.1.18:9001/web/all/event/log-datas',
    'getToken': 'http://192.168.1.18:9001/api/util-service/call-service/help-me/token/web',
};
...
function newAjaxSendDataNodejs(url, data) {
    ...
    $.ajax({
        ...
        success : function(response) {
            ...
            dataLogs.timeout = response.TIMEOUT;
            console.log(response);
        },
        error : function(error) {
            TIMEOUT = error.responseJSON.TIMEOUT;
            dataLogs.timeout = error.responseJSON.TIMEOUT;
...
function areYouSure() {
    if(allowPrompt){
        if (!areYouReallySure && true && !endedFlag) {
            ...
            dataLogs.isCloseTab = true;
            let event = 'beforeunload' || 'closeTab';
            finalSendData(event);
            resetDataLogs();
        }
    }else{
